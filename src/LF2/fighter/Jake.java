...
public class Jake extends Fighter implements ThreadListener {
		...
		if (state != "attack" && state != "defend" && state != "skill") {// 在ADS時不能走
			if (isStandX > 0) {// 當位移X>0時 為向右邊走
				...
				if (isMirror)// 是的話圖片重製
					frame = 0;
				frame += 0.1;// 圖片跟換速度
				frame %= (double) LF2Data.alljakeMoveImage.size();// 圖片張數以防溢出
				fighter = LF2Data.alljakeMoveImage.get((int) frame);// 為第幾張圖片
			} else if (isStandX < 0) {
				isMirror = true;
				if (!isMirror)
					frame = 0;
				frame += 0.1;
				frame %= LF2Data.alljakeMove_mirrorImage.size();
				fighter = LF2Data.alljakeMove_mirrorImage.get((int) frame);
			} else if (isStandY > 0) {
				frame += 0.1;
				if (isMirror) {
					frame %= LF2Data.alljakeMove_mirrorImage.size();
					...
					frame %= (double) LF2Data.alljakeMoveImage.size();
					fighter = LF2Data.alljakeMoveImage.get((int) frame);
				}
			} else if (isStandY < 0) {
				frame += 0.1;
				if (isMirror) {
	...
	public void Attack(Fighter enemy[]) {
		...
		if (!isJump && !isDefend && !isSkill) {// 狀態為JDS時無法A
			...
			state = "attack";
			if (isMirror) {
				fighter = LF2Data.alljakeAttack_mirrorImage.get(0);
			} else {
				fighter = LF2Data.alljakeAttackImage.get(0);
			}
		}
	}
	...
	public void Defend() {
		if (!isJump && !isAttack && !isSkill) {
			...
			state = "defend";
			if (isMirror)
				fighter = LF2Data.jakeDefense_mirrorImage;
			else
				fighter = LF2Data.jakeDefenseImage;
			isDefend = true;
		}
	}
	...
	public void Stand() {
		state = "stand";
		if (isMirror)
			fighter = LF2Data.alljakeMove_mirrorImage.get(0);
		else
			fighter = LF2Data.alljakeMoveImage.get(0);
		if (HP > 0) {
			if (HP <= f_HP)
	...
	public void drawImage(Graphics g, int winWidth, int winHeight) {
		...
		g.drawImage(LF2Data.alljakeMoveImage.get(0), (int) (winWidth * (0.001 + player % 4 * 0.2484)),
	...
	protected void attackState() {
		...
		for (int i = 0; i < 10; i++) { // 重複10次 比較會有動畫效果
			...
			if (isMirror) {
				...
				fighter = LF2Data.alljakeAttack_mirrorImage.get(1);// 換下一張圖
			} else {
				x += 0.0025;
				EnemyValue(0.08, 0.025, 0.04, 0.0025, 0, 0, 0.01, 0);
				fighter = LF2Data.alljakeAttackImage.get(1);
			}
			try {
				Thread.sleep(10);
			} catch (InterruptedException e) {
	...
	protected void skillState() {
		...
		case 1:
			if (MP >= 0.1) {
				...
				for (int i = 0; i <= 3; i++) {
					fighter = LF2Data.alljake_0mirrorImage.get(i);
					try {
						Thread.sleep(125);
					} catch (InterruptedException e) {
						e.printStackTrace();
		...
		case 2:
			if (MP >= 0.1) {
				...
				EnemyValue(0.08, 0.025, 0.04, -0.005, 0, 0, 0.025, 0);// 當打到敵人時的動作(哪個敵人,攻擊範圍x,y,z軸,x,y,z軸位移量,扣多少HP,MP)
				for (int i = 0; i <= 3; i++) {
					fighter = LF2Data.alljake_0Image.get(i);
					try {
						Thread.sleep(150);
					} catch (InterruptedException e) {
						e.printStackTrace();
			...
			if (MP >= 0.2) {
				...
				if (isMirror) {
					for (int i = 0; i <= 2; i++) {
						...
						fighter = LF2Data.alljake_1mirrorImage.get(i);
						try {
							Thread.sleep(150);
						} catch (InterruptedException e) {
							e.printStackTrace();
					...
					for (int i = 0; i <= 2; i++) {
						EnemyValue(0.08, 0.025, 0.04, 0.005, 0, 0, 0.05, 0);// 當打到敵人時的動作(哪個敵人,攻擊範圍x,y,z軸,x,y,z軸位移量,扣多少HP,MP)
						fighter = LF2Data.alljake_1Image.get(i);
						try {
							Thread.sleep(150);
						} catch (InterruptedException e) {
							e.printStackTrace();
