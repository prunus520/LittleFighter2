...
public class Capoo extends Fighter implements ThreadListener {
	...
	public void State() {
		if (state != "attack" && state != "defend" && state != "skill") {// 在ADS時不能走
			if (isStandX > 0) {// 當位移X>0時 為向右邊走
				...
				if (isRun) {
					fighter = LF2Data.capooRunImage;
				} else {
					if (isMirror)// 是的話圖片重製
						frame = 0;
					frame += 0.1;// 圖片跟換速度
					frame %= (double) LF2Data.allcapooMoveImage.size();// 圖片張數以防溢出
					fighter = LF2Data.allcapooMoveImage.get((int) frame);// 為第幾張圖片
				}
			} else if (isStandX < 0) {
				isMirror = true;
				if (isRun) {
					fighter = LF2Data.capooRun_mirrorImage;
				} else {
					if (!isMirror)
						frame = 0;
					frame += 0.1;
					frame %= LF2Data.allcapooMove_mirrorImage.size();
					fighter = LF2Data.allcapooMove_mirrorImage.get((int) frame);
				}
			} else if (isStandY > 0) {
				frame += 0.1;
				if (isMirror) {
					...
					if (isRun) {
						...
						frame %= (double) LF2Data.allcapooMoveImage.size();
						fighter = LF2Data.allcapooMoveImage.get((int) frame);
					}
				}
			} else if (isStandY < 0) {
				frame += 0.1;
	...
	public void Left() {
		isRun = false;
		state = "left";
		if (Math.abs(Keyboard.Time[player][0][0] - Keyboard.Time[player][0][1]) <= 200
	...
	public void Right() {
		isRun = false;
		state = "right";
		if (Math.abs(Keyboard.Time[player][1][0] - Keyboard.Time[player][1][1]) <= 500
	...
	public void Attack(Fighter enemy[]) {
		...
		if (!isJump && !isDefend && !isSkill) {// 狀態為JDS時無法A
			...
			state = "attack";
			if (isMirror) {
				fighter = LF2Data.capooAttack_mirrorImage.get(0);
			} else {
				fighter = LF2Data.capooAttackImage.get(0);
			}
		}
	}
	...
	public void Defend() {
		...
		if (!isJump && !isAttack && !isSkill) {
			...
			state = "defend";
			if (isMirror)
				fighter = LF2Data.capooDefense_mirrorImage;
			else
				fighter = LF2Data.capooDefenseImage;
		}
	}
	...
	public void Stand() {
		state = "stand";
		if (isMirror)
			fighter = LF2Data.allcapooMove_mirrorImage.get(0);
		else
			fighter = LF2Data.allcapooMoveImage.get(0);
		if (HP > 0) {
			if (HP <= f_HP)
	...
	public void drawImage(Graphics g, int winWidth, int winHeight) {
		...
		g.drawImage(LF2Data.allcapooMoveImage.get(0), (int) (winWidth * (0.001 + player % 4 * 0.2484)),
				(int) (winHeight * (0.0001 + player / 4 * 0.098)), (int) (winWidth * 0.075), (int) (winHeight * 0.095),
				null);
		// HP
	...
	protected void attackState() {
		...
		for (int i = 0; i < 10; i++) { // 重複10次 比較會有動畫效果
			...
			if (isMirror) {
				...
				fighter = LF2Data.capooAttack_mirrorImage.get(1);// 換下一張圖
			} else {
				x += 0.0025;
				EnemyValue(0.08, 0.025, 0.04, 0.0025, 0, 0, 0.01, 0);
				fighter = LF2Data.capooAttackImage.get(1);
			}
			try {
				Thread.sleep(10);
			} catch (InterruptedException e) {
	...
	public int skill() {
		...
		skill = skillCode(DRJ, 3);
		skill = skillCode(DLJ, 4);
		skill = skillCode(DDA, 5);
		skillKeyClear();
		return skill;
	}
	...
	protected void skillState() {
		...
		case 1:
			if (MP >= 0.1) {
				...
				fighter = LF2Data.allcapoo_0Image.get(0);
				try {
					Thread.sleep(200);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				fighter = LF2Data.allcapoo_0Image.get(1);
				try {
					Thread.sleep(200);
				} catch (InterruptedException e) {
					e.printStackTrace();
		...
		case 2:
			if (MP >= 0.1) {
				MP -= 0.1;
				EnemyValue(0.08, 0.025, 0.04, -0.005, 0, 0, 0.025, 0);// 當打到敵人時的動作(哪個敵人,攻擊範圍x,y,z軸,x,y,z軸位移量,扣多少HP,MP)
				fighter = LF2Data.allcapoo_0mirrorImage.get(0);
				try {
					Thread.sleep(200);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				fighter = LF2Data.allcapoo_0mirrorImage.get(1);
				try {
					Thread.sleep(200);
				} catch (InterruptedException e) {
					e.printStackTrace();
		...
		case 3:
			if (MP >= 0.2) {
				...
				fighter = LF2Data.allcapoo_1Image.get(0);
				try {
					Thread.sleep(200);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				fighter = LF2Data.allcapoo_1Image.get(1);
				try {
					Thread.sleep(200);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				fighter = LF2Data.allcapoo_1Image.get(2);
				try {
					Thread.sleep(200);
				} catch (InterruptedException e) {
					e.printStackTrace();
		...
		case 4:
			if (MP >= 0.2) {
				MP -= 0.2;
				EnemyValue(0.08, 0.025, 0.04, -0.005, 0, 0, 0.05, 0);// 當打到敵人時的動作(哪個敵人,攻擊範圍x,y,z軸,x,y,z軸位移量,扣多少HP,MP)
				fighter = LF2Data.allcapoo_1mirrorImage.get(0);
				try {
					Thread.sleep(200);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				fighter = LF2Data.allcapoo_1mirrorImage.get(1);
				try {
					Thread.sleep(200);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				fighter = LF2Data.allcapoo_1mirrorImage.get(2);
				try {
					Thread.sleep(200);
				} catch (InterruptedException e) {
					e.printStackTrace();
		...
		case 5:
			if (MP >= 0.25) {
				...
				if (isMirror) {
					for (int i = 0; i < 4; i++) {
						fighter = LF2Data.allcapoo_2mirrorImage.get(i);
						EnemyValue(0.08, 0.025, 0.04, -0.005, 0, 0, 0.02, 0);
						try {
							Thread.sleep(200);
						} catch (InterruptedException e) {
							e.printStackTrace();
					...
					for (int i = 4; i < 7; i++) {
						x -= 0.05;
						EnemyValue(0.08, 0.025, 0.04, -0.05, 0, 0, 0.05, 0);
						fighter = LF2Data.allcapoo_2mirrorImage.get(i);
						try {
							Thread.sleep(100);
						} catch (InterruptedException e) {
					...
					for (int i = 0; i < 4; i++) {
						EnemyValue(0.08, 0.025, 0.04, 0.005, 0, 0, 0.02, 0);
						fighter = LF2Data.allcapoo_2Image.get(i);
						try {
							Thread.sleep(200);
						} catch (InterruptedException e) {
							e.printStackTrace();
